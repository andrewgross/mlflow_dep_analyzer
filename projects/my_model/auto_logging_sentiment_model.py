...
class AutoLoggingSentimentModel(BaseModelV3):
    ...
    def _generate_and_log_requirements(self):
        """Generate smart requirements.txt for model dependencies using improved analyzer."""
        try:
            import os
            import tempfile

            from ..shared_utils.requirements_analyzer import (
                analyze_code_paths,
                collect_dependencies,
                load_requirements_from_file,
            )

            self.log_model_info("Generating smart requirements for model dependencies...")

            # Get the model file and its dependencies
            current_file = os.path.abspath(__file__)
            repo_root = os.path.dirname(os.path.dirname(os.path.dirname(current_file)))

            # First, find all local dependencies recursively
            local_deps = collect_dependencies(current_file, repo_root)
            self.log_model_info(f"Found {len(local_deps)} local dependencies")
            ...
            # Analyze only the code paths that will be included with the model
            code_paths = [current_file, shared_utils_dir]

            # Generate requirements, excluding packages from base requirements
            requirements = analyze_code_paths(code_paths, existing_requirements=base_requirements)

            # Create temporary requirements file
            with tempfile.NamedTemporaryFile(mode="w", suffix=".txt", delete=False) as f:
                f.write("# Auto-generated minimal requirements for model execution\n")
                f.write("# Contains only packages needed to run this specific model\n")
                f.write("# Generated by improved requirements analyzer\n")
            ...
            # Log requirements count as metric
            mlflow.log_metric("requirements_count", len(requirements))
            mlflow.log_metric("base_requirements_count", len(base_requirements))
            mlflow.log_metric("local_dependencies_count", len(local_deps))

            # Log requirements as parameter (truncated if too long)
            ...
            self.log_model_info(f"  (Excluded {len(base_requirements)} base packages)")
            for req in requirements:
