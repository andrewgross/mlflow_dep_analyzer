...
class TestMLflowIntegration:
    ...
    def test_model_with_spark_artifacts(self, mlflow_server, spark_session, sample_data, test_predictions_data):
        ...
        df = spark_session.createDataFrame(list(zip(texts, categories, strict=False)), ["text", "category"])
        ...
        with mlflow.start_run():
            log_model_with_code_paths(
                model=model, artifact_path="sentiment_model_with_spark", artifacts={"category_indexer": indexer_path}
            )
    ...
    def test_multiple_models_with_different_code_versions(self, mlflow_server, sample_data, test_predictions_data):
        ...
        # Save first version
        with mlflow.start_run():
            log_model_with_code_paths(model=model1, artifact_path="sentiment_model")
        ...
        # Save second version
        with mlflow.start_run():
            log_model_with_code_paths(model=model2, artifact_path="sentiment_model")
